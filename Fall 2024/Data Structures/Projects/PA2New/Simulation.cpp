...
//constructor
Simulation::Simulation(int L, int gridSize, int V, const std::string& logFileName,int perCoins,int perNothing,int perGoomba,int perKoopa,int perMushroom) 
: world(L,gridSize), mario(V, 0, 0, 0)
    {
        logFile.open(logFileName, std::ios::app);
        world.populateWorld(perCoins, perNothing, perGoomba, perKoopa, perMushroom);
    ...
    //run simulation
    void Simulation::simulate(FileProcessor& fileProcessor)
    {
        ...
        world.levels->placeItem(marioRow, marioCol,'H');
        while (mario.lives > 0 && currentLevel < world.numLevels) 
        {
            //stop endlessness
            x++;
            if (x == 500){
            ...
            world.levels->display(fileProcessor);
            logCurrentState(currentLevel, marioRow, marioCol);
            char item = world.levels[currentLevel].levelGrid[marioRow][marioCol];
            ...
            world.levels->placeItem(marioRow, marioCol,'H');
            world.levels->placeItem(marioRow--, marioCol--,'x');
        }

        logResult(currentLevel == world.numLevels);
